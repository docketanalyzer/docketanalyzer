...
def page_needs_ocr_handled(page, text):
    try:
...
def extract_with_ocr_handled(page, strip_margin):
    try:
        return extract_with_ocr(page, strip_margin)
    except UnicodeDecodeError as e:
        print(f"UnicodeDecodeError: {e}")
    except RecursionError as e:
        print(f"RecursionError: {e}")
...
def extract_text_handled(page):
    try:
        return page.extract_text()
    except pdfminer.psparser.PSSyntaxError as e:
        print(f"pdfminer.psparser.PSSyntaxError: {e}")
    return ''
    ...
    try:
        with pdfplumber.open(path) as pdf:
            ...
            for i, page in enumerate(pdf.pages):
                #print(f"Processing: {i + 1}/{num_pages} ({path})")
