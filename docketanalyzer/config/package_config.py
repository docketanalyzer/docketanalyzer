...
class PackageConfig():
    def __init__(self, path, config_keys, cli_command, autoload_env=True, available_extensions=[]):
    ...
    def update(self, group=None, reset=False):
        ...
        try:
            ...
            for key in self.keys:
                if key.required_extensions is None or any([x in self.available_extensions for x in key.required_extensions]):
